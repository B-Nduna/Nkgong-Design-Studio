(function (factory){
    if (typeof define === "function" && define.amd){
        //AMD. Regster as an anonymous module.
        define["jquerry"], factory;
    } else if (typeof module === "object" && module.exports){
        module.exports = function (root, jQuery){
            // Node/CommonJS
            if (jQuery === undefined){
                //require (jQuery) returns a factory that requires window to
                //build a jQuerry instance, we normalize how we use modules
    ...
    $.fn.tilt = function (options){
        ...
        /**
         * transition only n mouse leave and mouse enter
         */
        ...
        const mouseMove = function (event){
            this.getMousePositions = getMousePositions(event);
        ...
        const getValues = function (){
            ...
            //x or y position inside instance / width of instance = percentage of positin inside instance * max tilt value
            const tiltX = (
    ...
    const updateTransforms =function (){
        ...
        if (this.reset){
            ...
            $(this).css(
                "transform",
                'perspective(${this.setings.perspective}px rotateX(${this.settings.disableAxis === "x" ? 0 : this.transforms.tiltY}deg) rotateY(${this.settings.disableAxis === "y" ? 0 : this.transforms.tiltX}deg) scal3d(${this.settings.scale},${this.settings.scale},${this.setings.scale})'
            );

            //Rotate glare is enabled
            if (this.settings.glare){
                this.glareElement.css(
                    "transform",
        ...
        const prepareGlare = function(){
         ...
         // If option pre-order is enabled we assume all html/css is presentfor an optial glare effect.
         if (!glarePrerender)
        ...
        // Abstracted re-unusable glare styles
        const stretch = {
            position: "absolute",
            top: "0",
            left: "0",
        ...
        return this.each(function (){
             ...
             * can be set through data attributes or as per parameter.
             * @type {*}
             */
            this.settings = $.extend(
                {
                    ...
                    ? $(this).data("tilt-ttransition")
                    :true,
                    doubleAxis: $(this).is("[data-tilt-double-axis]")
                    ? $(this).data("tilt-double-axis")
                    :null,
            ...
            //Add deprecation warning & set doubleAxis to deprecated acis setting
            if (this.settings.axis !== null) {
                console.warn(
                    ""
                );
                this.settings.doubleAxis = this.setting.axis;
            }
